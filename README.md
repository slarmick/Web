...
- Реализовать штрафное задание: обе системы (RabbitMQ и Kafka) в одном проекте
### Выполненные этапы:

#### Этап 1: Настройка Docker Compose с RabbitMQ и Kafka
- Добавление сервисов RabbitMQ, Kafka и Zookeeper в docker-compose.yml
- Настройка RabbitMQ 3.13-management-alpine с веб-интерфейсом (порт 15672)
- Конфигурация Kafka 3.7 с Zookeeper для координации
- Создание единой сети для взаимодействия всех контейнеров

#### Этап 2: Создание класса QueueManager для работы с очередями
- Реализация методов для работы с RabbitMQ (AMQP протокол)
- Интеграция с Kafka через библиотеку kafka-php
- Двухуровневая архитектура: основные очереди и очереди ошибок
- Обработка deprecated warnings и буферизация вывода

#### Этап 3: Интеграция очередей в процесс регистрации
- Модификация process.php для асинхронной отправки данных
- Отправка данных в обе системы (RabbitMQ и Kafka) одновременно
- Обработка ошибок с автоматическим перемещением в очередь ошибок
- Логирование всех операций с очередями

#### Этап 4: Создание Queue Dashboard
- Разработка queue_dashboard.php для мониторинга очередей
- Отображение статистики подключений к RabbitMQ и Kafka
- Просмотр последних сообщений в реальном времени
- Визуализация статуса систем и количества сообщений

#### Этап 5: Реализация Redis Dashboard
- Две очереди/топика: основная (lab7_main_queue) и для ошибок (lab7_error_queue)
- Обработка ошибок с перемещением сообщений в очередь ошибок
- Статистика по количеству сообщений в обеих очередях
- Отказоустойчивость при недоступности одной из систем

### Технологии использованные в пятой лабе:
- 🐇 **RabbitMQ 3.13** - message broker с поддержкой AMQP протокола
- 🦊 **Apache Kafka 3.7** - распределенная streaming платформа
- 📊 **Message Queues** - асинхронная обработка задач
- ⚡ **Async Processing** - неблокирующая обработка данных
- 🐘 **PHP 8.2** с расширениями Redis и Sockets
- 📦 **PHP-AMQPLib** - библиотека для работы с RabbitMQ
- 🔧 **Kafka-PHP** - клиент для Apache Kafka
- 🐳 **Docker Compose** - оркестрация многоконтейнерного приложения

### Архитектура системы:
Пользователь → Форма → Валидация → MySQL → Redis → RabbitMQ → Kafka → Ответ
                       ↓          ↓         ↓         ↓
                    Сессии     Файл     Dashboard   Worker

### Панели управления:
- **Queue Dashboard:** http://localhost:8080/queue_dashboard.php
- **RabbitMQ Admin:** http://localhost:15672 (логин: guest, пароль: guest)
- **Redis Dashboard:** http://localhost:8080/redis-dashboard.php
...
# Просмотр логов
docker logs lab7_php --tail 20
docker logs lab7_kafka --tail 10

# Проверка RabbitMQ
curl http://localhost:15672/api/queues

# Запуск тестового worker
docker exec -it lab7_php php /var/www/html/queue_worker.php
```

### Результат работы:
Создана полноценная система асинхронной обработки данных с использованием двух брокеров сообщений (RabbitMQ и Kafka). Реализована отказоустойчивая архитектура с обработкой ошибок и мониторингом в реальном времени. Каждая регистрация на мастер-класс теперь обрабатывается асинхронно через очереди, что повышает производительность и надежность системы. Все компоненты интегрированы в существующую архитектуру проекта и доступны через веб-интерфейс.